...
int main(void) {
  ...
  uarths_init();   //高速串口  使用PLL0
  io_set_power();  // io电源
  io_mux_init();   // io功能
  plic_init();     // PLIC 初始化外部中断
  /* flash init */
  printf("flash init\n");
  w25qxx_init(3, 0);  // SPI3
  ...
  /* LCD init */
  printf("LCD init\n");
  lcd_init();  // LCD SPI0  2.5M  SPI_FF_OCTAL = 3
#if BOARD_LICHEEDAN
#if OV5640
...
#else
#if OV5640
  lcd_set_direction(DIR_YX_RLUD);  //显示方向设置
#else
  ...
  printf("DVP init\n");
#if OV5640
  dvp_init(16);                              // DVP 初始化，sccb长度16
  dvp_set_xclk_rate(12000000);               // dvp时钟
  dvp_enable_burst();                        //突发传输模式
  dvp_set_output_enable(0, 1);               //使能输出模式到AI
  dvp_set_output_enable(1, 1);               //使能输出模式到内存
  dvp_set_image_format(DVP_CFG_RGB_FORMAT);  //设置图像接受模式为rgb
  dvp_set_image_size(320, 240);              //图像采集尺寸 320240
  ov5640_init();                             //摄像头初始化
#else
  dvp_init(8);
  ...
  image_init(&kpu_image);  // KPU容量
  display_image.pixel = 2;
  display_image.width = 320;
  display_image.height = 240;
  image_init(&display_image);  //显示image容量
  dvp_set_ai_addr(
      (uint32_t)kpu_image.addr, (uint32_t)(kpu_image.addr + 320 * 240),
      (uint32_t)(kpu_image.addr + 320 * 240 * 2));     // DVP R/G/B 地址设置
  dvp_set_display_addr((uint32_t)display_image.addr);  // DVP rgb 显示地址
  dvp_config_interrupt(DVP_CFG_START_INT_ENABLE | DVP_CFG_FINISH_INT_ENABLE,
                       0);  //图像采集开始和结束中断 禁止
  dvp_disable_auto();       //禁用自动接收图像模式
  /* DVP interrupt config */
  printf("DVP interrupt config\n");
  plic_set_priority(IRQN_DVP_INTERRUPT, 1);  //中断优先级1 ，dvp
  plic_irq_register(IRQN_DVP_INTERRUPT, dvp_irq, NULL);  //注册外部中断函数。
  plic_irq_enable(IRQN_DVP_INTERRUPT);                   //使能外部中断。
  /* init face detect model */
  if (kpu_load_kmodel(&face_detect_task, model_data_align) !=
      0) {  //加载kmodel，需要与nncase 配合使用。
    printf("\nmodel init error\n");
    while (1)
      ;
  }
  ...
  face_detect_rl.anchor_number = ANCHOR_NUM;  // 5
  face_detect_rl.anchor = anchor;
  // float anchor[ANCHOR_NUM * 2] = {1.889,    2.5245, 2.9465,   3.94056,
  //                                3.99987,  5.3658, 5.155437, 6.92275,
  //                                6.718375, 9.01025};
  ...
  uint64_t time_now = sysctl_get_time_us();  //开机到现在时间
  int time_count = 0;
  while (1) {
    g_dvp_finish_flag = 0;
    dvp_clear_interrupt(DVP_STS_FRAME_START |
                        DVP_STS_FRAME_FINISH);  //清楚中断  开始转换和结束转换
    dvp_config_interrupt(DVP_CFG_START_INT_ENABLE | DVP_CFG_FINISH_INT_ENABLE,
                         1);  //图像开始中断，结束中断 使能
    ...
    size_t output_size;
    //                      任务句柄，索引值，            结果，      大小
    kpu_get_output(&face_detect_task, 0, (uint8_t **)&output,
                   &output_size);  //获得kpu最终处理结果
    //  face_detect_rl.input = output;  // region_layer_t,数据
/*
    printf("output_size %ld\n", output_size);
    uint32_t temp;
    for (temp = 0; temp < output_size; temp++) {
      printf("%f\n", *(output++));
    }*/
